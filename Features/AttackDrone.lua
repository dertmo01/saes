...
function SpawnLoop()
    ...
    task.spawn(function()
        while AttackDroneEnabled do
            -- If we already have a live target, just wait
            if CurrentTarget and CurrentTarget.Parent then
                task.wait(0.5)
                continue
            end

            -- ✅ Target just died (or no target yet) — scan immediately before flying anywhere
            local Found = FindBestDrone()
            if Found and Found.Parent then
                print("[AttackDrone] ✅ Target destroyed → Found next target immediately (Top " .. tostring(GetDronePriority(Found)) .. ")")
                CurrentTarget = Found
                CurrentTargetPriority = GetDronePriority(Found)
                CleanupMovers()
                StartFollow()
                -- Wait until this target is gone
                while AttackDroneEnabled and CurrentTarget and CurrentTarget.Parent do
                    task.wait(0.3)
                end
                print("[AttackDrone] Mob Cleared → Scanning again...")
                task.wait(0.1)
                continue
            end

            -- No drone found nearby — fly to spawn to look
            print("[AttackDrone] No drone found nearby → Flying to Spawn " .. CurrentSpawnIndex)
            local CurrentSpawn = (CurrentSpawnIndex == 1) and SPAWN_POSITION_1 or SPAWN_POSITION_2
