...
local function StopDrone()
    if _G.YOKUDO_AttackDrone and _G.YOKUDO_AttackDrone.IsEnabled() then
        ...
        DroneStarted = false
        print("[FarmingManager] ✅ Drone Stopped")
    end
end
...
local function OnVIPTPComplete()
    ...
    if BestEgg then
        -- Priority 1: another egg available → farm it
        ...
        -- Priority 2: no egg → try drone (day only)
        local Phase = GetPhase()
        if Phase == "Day" and _G.YOKUDO_AttackDrone and not _G.YOKUDO_AttackDrone.IsEnabled() then
            print("[FarmingManager] No Egg → Start Drone (Day)")
            _G.YOKUDO_AttackDrone.Start()
            DroneStarted = true
        else
            -- No egg, no drone applicable → idle, wait for next egg
            print("[FarmingManager] No Egg → Idle, waiting for next egg")
        end
    end
end
...
local function NightLoop()
    ...
    while FarmingEnabled do
        ...
        if BestEgg then
            -- Egg found at night → stop drone, fly to safe, wait for day, VIPTP
            ...
            -- No egg at night → drone not useful at night, just idle
            if DroneStarted then
                StopDrone()
            end
        end
...
local function DayLoop()
    ...
    while FarmingEnabled do
        ...
        if BestEgg then
            -- Priority 1: egg → stop drone, go farm
        ...
        elseif _G.YOKUDO_AttackDrone and not _G.YOKUDO_AttackDrone.IsEnabled() and not WaitingForVIPTP then
            -- Priority 2: no egg, day → use drone
